...
class ankix_dbo
{
    ...
    {/*{{{*/
        ...
        foreach ($data as $col => $value) {
            $ups[] = "`$col`='".ankix_real_escape_string($value)."'";
        }   
    ...
    {/*{{{*/
        ...
        foreach ($data as $col => $value) {
            $cols[] = "`$col`";
            $vals[] = "'".ankix_real_escape_string($value)."'";
        }   
	...
	{/*{{{*/
		...
		foreach ($values as &$value) {
			$v = ankix_real_escape_string($value);
			$sql.= $sp."'$v'";
			$sp = ",";
		}
		$sql.=")";
...
/* 去除数组中的数字下标 */
function ankix_array_unset_int_key(array &$arr)
{/*{{{*/
 ...
 * 此版本无需事先连接数据库
 **/
function ankix_real_escape_string($str)
{/*{{{*/
