...
class ankix_env
{
    ...
    {/*{{{*/
        global $_G;
		$_G['siteurl'] = preg_replace("/source\/plugin\/ankix/i","", $_G['siteurl']);
    ...
    {/*{{{*/
        ...
        if ($charset=='gbk') {
            $sitename = ankix_utils::toutf8($sitename);
        }
        return $sitename;
    }/*}}}*/
    ...
    {/*{{{*/
        return self::get_siteurl().'/source/plugin/ankix';
    }/*}}}*/
    ...
    {/*{{{*/
        if (!self::$_log_obj) {
            ...
            self::$_log_obj = new ankix_log($logcfg);
        }   
    ...
    {/*{{{*/
        if (!isset(self::$_pdo_objs[$key])) {
            $config = C::m("#ankix#ankix_setting")->get();
            if (!isset($config[$key])) {
            ...
            $pass = $dbconf['pass'];
            $db   = $dbconf['db'];
            self::$_pdo_objs[$key] = new ankix_dbo($host,$port,$user,$pass,$db);
        }
