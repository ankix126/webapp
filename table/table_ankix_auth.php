 ...
 * 用户权限表
 **/
class table_ankix_auth extends discuz_table
{
    public function __construct() {
		$this->_table = 'ankix_auth';
		$this->_pk = 'uid';
		parent::__construct();
	}
    ...
    {/*{{{*/
        ...
        $auth = ankix_validate::getNCParameter('auth','auth','integer');
        $ctime = date("Y-m-d H:i:s");
        $sql = "INSERT INTO ".DB::table('ankix_auth')." (uid,auth,ctime) VALUES ('$uid','$auth','$ctime') ".
	...
	{/*{{{*/
        ...
        $auth  = ankix_validate::getNCParameter('auth','auth','integer');
        $key   = ankix_validate::getNCParameter('key','key','string',128);
        $sort  = ankix_validate::getOPParameter('sort','sort','string',1024,'uid');
...
EOF;
        ...
        foreach ($return["root"] as &$row) {
            $row['groupname'] = C::m('#ankix#ankix_usergroup')->grouptitle($row['groupid']);
