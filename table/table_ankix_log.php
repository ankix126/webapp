 ...
 * 插件访问日志表
 **/
class table_ankix_log extends discuz_table
{
	public function __construct() {
		$this->_table = 'ankix_log';
		$this->_pk = 'logid';
		parent::__construct();
	}
	...
	{/*{{{*/
		...
		$sday  = ankix_validate::getNCParameter('sday','sday','string'); 
		$eday  = ankix_validate::getNCParameter('eday','eday','string'); 
		$key   = ankix_validate::getNCParameter('key','key','string'); 
        $sort  = ankix_validate::getOPParameter('sort','sort','string',1024,'logtime');
		...
		$table_ankix_log = DB::table($this->_table);
		$table_common_member = DB::table('common_member');
		$sql = <<<EOF
SELECT SQL_CALC_FOUND_ROWS a.*,b.username,b.email
FROM $table_ankix_log as a LEFT JOIN $table_common_member as b ON a.uid=b.uid
...
FROM $table_ankix_log WHERE $where GROUP BY vday ORDER BY logtime ASC
EOF;
