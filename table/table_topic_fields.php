 ...
 * @usage: C::t('#ankix#topic_fields')->fun();
 **/
class table_topic_fields
{
    private $table = "topic_fields";
    private $pdo;
...
EOF;
    ...
    {/*{{{*/
        global $_G;
        $uid = $_G['uid'];
        $fid = ankix_validate::getNCParameter('fid','fid','integer');
        $record = array (
            'fname' => ankix_validate::getNCParameter('fname','fname','string',50),
            'ftype' => ankix_validate::getNCParameter('ftype','ftype','integer'),
            'fdesc' => ankix_validate::getNCParameter('fdesc','fdesc','string',1024),
        ); 
        if ($fid==0) {
            ...
            return $this->pdo->update($this->table,$record,array("fid='$fid'"));
        }
    }/*}}}*/
