define(function(require){
    /* generated by mengma @2018-12-16 00:39 */
    var dialogid = 'dialog-'+mwt.genId();
    var fieldFactory = require('common/field_factory');
    var form,dialog,params,callback;
    
    function init_dialog() 
    {/*{{{*/
        //1. new form
        form = new MWT.Form();
        form.addField('name',new MWT.TextField({
            type        : 'text',
            render      : 'fm-name'+dialogid,
            value       : '', 
            placeholder : '',
            empty       : false,
            errmsg      : "请输入单元名称,不超过1024个字符",
            checkfun    : function(v){return v.length<=1024;}
        }));
        form.addField('cover_img',fieldFactory.get(3,'fm-cover_img'+dialogid,''));
        form.addField('description',new MWT.TextField({
            type        : 'text',
            render      : 'fm-description'+dialogid,
            value       : '', 
            placeholder : '',
            empty       : true,
            errmsg      : "请输入单元描述,不超过1024个字符",
            checkfun    : function(v){return v.length<=1024;}
        }));

        //2. new dialog
        dialog = new MWT.Dialog({
            title     : '对话框',
            form      : form,
            style     : 'width:500px;height:300px',
            bodyStyle : 'padding:10px;',
            body : '<table class="mwt-formtab">'+
               '<tr>'+
                 '<th width="90">'+lan(['unit','name'])+' <b style="color:red">*</b></th>'+
                 '<td><div id="fm-name'+dialogid+'"></div></td>'+
                 '<td class="tips" width="20"></td>'+
               '</tr>'+
               '<tr>'+
                 '<th>'+lan('cover')+' <b style="color:red">*</b></th>'+
                 '<td><div id="fm-cover_img'+dialogid+'"></div></td>'+
                 '<td class="tips"></td>'+
               '</tr>'+
               '<tr>'+
                 '<th>'+lan('description')+'</th>'+
                 '<td><div id="fm-description'+dialogid+'"></div></td>'+
                 '<td class="tips"></td>'+
               '</tr>'+
            '</table>',
            buttons : [
                {label:lan('submit'),cls:'mwt-btn-primary',handler:submitClick},
                {label:lan('cancel'),type:'close',cls:'mwt-btn-default'}
            ]
        });
        //3. dialog open event
        dialog.on('open',function(){
            form.reset();
            if (params.id) {
                dialog.setTitle(lan(["edit","unit"]));
                form.set(params);
            } else {
                dialog.setTitle(lan(["add",'unit']));
            }
        });
    }/*}}}*/

    var o={};
    o.open=function(_params,_callback){
        params   = _params;
        callback = _callback;
        if (!dialog) init_dialog();
        dialog.open();
    };  

    /////////////////////////////////////
    // 提交按钮点击事件
    function submitClick() {
        var data = form.getData();
        try {
            data.id = params.id;
            data.tid = params.tid;
            ajax.post('topic&action=saveUnit',data,function(res){
                if (res.errno!=0) mwt.notify(res.errmsg,1500,'danger');
                else {
                    dialog.close();
                    if (callback) callback();
                }   
            }); 
        } catch (e) {
            mwt.notify(e,1500,'danger');
        }
    }

    return o;
});
